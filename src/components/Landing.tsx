            ...
            <div className="w-full max-w-3xl">
                <div className="text-center mb-2">
                    <span className="chip">
                        <button
                            type="button"
                            className="lifter-secret"
                            onClick={onLifterClick}
                            aria-label="Plate Math"
                        >
                            🏋️‍♀️
                        </button>
                        <span>NUMERICAL PROTEIN 🏋️‍♀️</span>
                        {settings.devMode && (
                            <span className="dev-badge">DEV · 5s sprint</span>
                        )}
                    </span>
                </div>
                <h1 className="fancy-headline text-6xl md:text-7xl text-center mt-3 text-ink-800">
                    Plate <span className="text-peach-400">Math</span>.fun
                </h1>
                <p className="text-center text-lg mt-3 max-w-xl mx-auto text-ink-700/80">
                    Calculate the bar total and prove yourself!
                </p>
                ...
                <div className="card p-6 md:p-8 mt-4">
                    <button
                        onClick={() => {
                            posthog.capture("game_started", { mode: "sprint" });
                            onStart("sprint");
                        }}
                        className="btn-chunky btn-chunky--mint w-full flex-col py-7"
                        type="button"
                    >
                        <div className="text-3xl tracking-wide">PLAY</div>
                    ...
                    <div className="mt-5 flex justify-center">
                        ...
                        >
                            ...
                            <span>Options</span>
                            <span
                                className={`options-chevron ${optionsOpen ? "options-chevron--open" : ""}`}
                                aria-hidden
                            >
                                ▾
                            </span>
                            ...
                            <Toggle
                                ...
                                onChange={(v) => {
                                    posthog.capture("settings_changed", {
                                        setting: "input_mode",
                                        value: v ? "choice" : "numpad",
                                    });
                                    setSettings({
                                        inputMode: v ? "choice" : "numpad",
                                    });
                                }}
                            />
                            <Toggle
...
function NameEditor({ name, playerId, onSave }: NameEditorProps) {
    ...
    function commit() {
        ...
        posthog.capture("leaderboard_name_saved", {
            is_new: name.trim().length === 0,
        });
        onSave(cleaned);
    ...
    return (
        ...
        >
            ...
            <button
                type="submit"
                className="btn-chunky btn-chunky--mint name-editor__btn"
            >
                Save
            </button>
        </form>
